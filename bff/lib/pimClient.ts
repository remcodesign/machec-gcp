...
function stringRecord(value: unknown): Record<string, string> {
    ...
    return Object.fromEntries(
        Object.entries(value).map(([key, entry]) => [key, stringValue(entry, `attributes.${key}`)]),
    );
}
...
function categoryValue(value: unknown): Category {
    ...
    if (
        ...
        throw new PimClientError("PIM response field filterable_attributes is invalid.");
    }
...
function paginationMeta(value: unknown, count: number): PaginationMeta {
    ...
    const currentPage = typeof value.current_page === "number" ? value.current_page : 1;
    const lastPage = typeof value.last_page === "number" ? value.last_page : currentPage;
    const perPage = typeof value.per_page === "number" ? value.per_page : DEFAULT_PAGE_SIZE;
...
function productList(value: unknown): ProductListResponse {
    ...
    const data = payload.data.map(productValue).filter((product) => product.status === "published");
...
async function request(path: string, query?: URLSearchParams): Promise<unknown> {
    ...
    try {
        ...
        if (!response.ok) {
            throw new PimClientError(`PIM catalog request failed with HTTP ${response.status}.`, response.status);
        }
        ...
        const clientError = error instanceof PimClientError
            ? error
            : new PimClientError(error instanceof Error ? error.message : "PIM catalog request failed.");
...
export async function getProducts(filters: CatalogFilters = {}): Promise<ProductListResponse> {
    ...
    return productList(await request("products", filtersQuery({ ...filters, status: "published" })));
}
